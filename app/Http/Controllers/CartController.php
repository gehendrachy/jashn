...
class CartController extends Controller
{
    ...
    public function update_cart(Request $request)
    {
        ...
        $cart = (array)session()->get("cart");
        $cart_total_price = collect($cart)->pluck('sub_total')->sum();
    ...
    public function checkout()
    {
        ...
        if (count($cart_offers) != 0) {
            foreach ($cart_offers as $cart_offer) {
                ...
                foreach ($valid_ordered_products as $valid_offer_product) {
                    ...
                    $cart_total_offer_price += $item_discount_amount;
        ...
        return view('checkout', compact('cart', 'cart_total_price', 'user', 'billing_details', 'shipping_details', 'countries', 'cart_total_weight', 'coupon_details', 'cart_total_offer_price'));
    }

    public function checkoutSaveDetails(Request $request){
        ...
        $this->validate($request, 
                    [
                        ...
                        'billing_email' => 'required_unless:same_address,1|email|max:225',
                        // 'billing_country_name' => 'max:225|'.
                        //                             Rule::requiredIf(function(){
                        //                                 return !isset($this->request->same_address);
                        //                             }),
                        'billing_country_id' => Rule::requiredIf(function(){
                                                    return isset($this->request->same_address) && $this->request->same_address == 1;
                                                }),
                        // 'billing_state_name' => 'max:225|'.
                        //                             Rule::requiredIf(function(){
                        //                                 return !isset($this->request->same_address);
                        //                             }),
                        'billing_state_id' => Rule::requiredIf(function(){
                                                    return isset($this->request->same_address) && $this->request->same_address == 1;
                                                }),
                        ...
                        //                                 return !isset($this->request->same_address);
                        //                             }),
                        'billing_district_id' => Rule::requiredIf(function(){
                                                    return isset($this->request->same_address) && $this->request->same_address == 1;
                                                }),
                        ...
                        //                                 return !isset($this->request->same_address);
                        //                             }),
                        'billing_city_id' => Rule::requiredIf(function(){
                                                    return isset($this->request->same_address) && $this->request->same_address == 1;
                                                }),
        ...
        $offer_price = $this->calculate_offer();
        session()->put('offer_price', $offer_price);
        ...
        session()->put('cart_total_weight', $cart_total_weight);
    ...
    public function checkoutConfirmation()
    {
        ...
        $details = session()->get('checkout_details');
        // dd($details);
        // dd($details['shipping']['district_id']);

        $cart = session()->get('cart');
        $cart_total = collect($cart)->sum('sub_total');
        $cart_total_weight = session()->get('cart_total_weight');
        $coupon_details = (array)session()->get('coupon_details');
        $cart_total_offer_price = session()->get('offer_price');
        ...
        $district = District::find($details['shipping']['district_id']);

        // dd($instock_cart_total_weight.'---'.$preorder_cart_total_weight);
        ...
        // dd($total_delivery_charge);
        // $cart_total_weight = session()->get('cart_total_weight');
        $shipping_cost = $total_delivery_charge;
        
        if (Auth::check()){
          
            $total_delivered_orders = auth()->user()->ordered_products()->where([['ordered_products.status',5]])->count();
    
            $total_canceled_orders = auth()->user()->ordered_products()->where([['ordered_products.status',6]])->count();
            $total_orders = auth()->user()->ordered_products()->count();
        ...
        // dd($details);
        return view('checkout-confirmation', compact('details', 'cart', 'cart_total', 'cart_total_weight', 'coupon_details', 'cart_total_offer_price', 'shipping_cost', 'success_rate'));
    }
    ...
    public function get_cities(Request $request)
    {
        ...
        $offer_amount = $this->calculate_offer();

        $total_price = $cart_total_price + $shipping_cost - $discount_amount - $offer_amount;
        ...
        if ($request->action == 'apply_coupon') {
            ...
            if ($coupon) {
                ...
                if ($todayDate >= $coupon->start_date && $todayDate <= $coupon->expire_date ) {
                    ...
                    if ($valid_sub_total_price < $coupon->minimum_spend) {
                    ...
                    if ($valid_total_ordered_qty < $coupon->minimum_quantity) {
                    ...
                    $cart_total_price = $cart_total_price - $offer_price - $discount_amount + $shipping_cost;
    ...
    public function calculate_offer()
    {
